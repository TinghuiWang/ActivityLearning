...
from actlearn.models.MultiLayerPerceptron import MultiLayerPerceptron
...
def run_test(act_feature):
    ...
    model = MultiLayerPerceptron(input=input_x, numpy_rng=numpy_rng,
                                 n_in=x.shape[1], n_out=num_classes,
                                 n_hidden=num_hidden)
...
def casas_train(x, y, model):
    ...
    :return: None
    """
    assert(type(model) == MultiLayerPerceptron)
    ...
    model.do_training_sgd(data=x_tensor, label=y_tensor,
                          ...
                          learning_rate_array=[0.1, 0.1, 0.1, 0.1], num_epochs=10)
